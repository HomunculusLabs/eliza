 ...
 * paths (missing room, already titled, no user message, too-short text) and
 * the fallback-title intent classification, including the word-boundary
 * regression where "history of rome"-class first messages were permanently
 * retitled to a generic bucket (#30122). Deterministic bun:test with the
 * repositories and logger mocked; no database, no runtime.
 */
...
describe("generateFallbackTitle — word-boundary regression (#30122)", () => {
  ...
  // A plain \b boundary is ASCII-only, so a Unicode letter after the keyword
  // still counted as a boundary transition and matched the prefix.
  test("does not match a Unicode-letter extension of an intent keyword", async () => {
    await expect(titleFor("Supérieur service nous attend")).resolves.toBe(
      "Supérieur service nous attend",
    );
    await expect(titleFor("Hiện tại đường truyền yếu")).resolves.toBe("Hiện tại đường truyền yếu");
  });
});
