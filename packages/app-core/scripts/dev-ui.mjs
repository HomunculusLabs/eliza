...
// Fail fast on a missing Node 24+ BEFORE any dev child spawns: the supervised
// Vite child requires it (see lib/dev-ui-vite.mjs), and discovering the
// actionable ELIZA_NODE_PATH error after the API child is already running
// would defeat the fail-fast property.
resolveNodeRuntimePath(process.env);
...
function startVite() {
  ...
  // Vite's HTTP and WebSocket proxy depends on Node socket semantics that Bun
  // does not implement (`socket.destroySoon` and HTTP upgrade handling), so the
  // supervised Vite child is always resolved to the repository-required Node
  // 24+ runtime even when this orchestrator runs under Bun. resolveViteCommand
  // keeps its generic runtime default for the standalone launchers
  // (packages/app dev, dev:shared, dev:desktop), which may stay Bun-backed.
