...
| media-store WAV write (`persistMeetingAudio` → host `MEDIA_WRITE_PORT_SERVICE` port, single content-addressed store) | filesystem / served media dir | `meeting-transcript-writer.test.ts` (`persistMeetingAudio`) | real `fs` port fixture into a `mkdtempSync` temp dir via `ELIZA_STATE_DIR`; content-addressed + idempotent + typed fail-closed asserted | none |
