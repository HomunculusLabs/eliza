...
describe("textStatesExplicitUndatedTodo", () => {
  ...
  it.each([
    // #30975: a later, separate calendar operation in the same plain-prose
    // message owns its own times and must not correct or complete the todo's
    // undated authority.
    ...
    "keeps undated authority when a separate plain-prose operation supplies its own time in %p",
    (text) => {
      expect(textStatesExplicitUndatedTodo(text)).toBe(true);
      expect(textContradictsExplicitUndatedTodo(text)).toBe(false);
    },
  ...
  it.each([
    // Corrections and cross-references that target THIS todo still contradict
    // its no-date directive; pronoun- and definite-noun-led transforms never
    // introduce a separate item.
    ...
    "add buy milk with no due date; schedule the todo on friday",
    "add buy milk with no due date; also schedule the first todo tomorrow",
  ])(
